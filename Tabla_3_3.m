...
u1=.1*[0 12* sign(cos(0.12*t))];u1p=[0 diff(u1)/At];
u2=100*[0 1.5* sign(cos(1.3*t+1))];u2p=[ 0 diff(u2)/At];
for jj=1:Kmax-1
    y1pp=(1/4)*(-u1p(jj)+2*u1(jj)+2*u2p(jj)+u2(jj)-4*y1p(jj)-y1(jj));    
    y2pp=(1/4)*(-3*u1(jj)+2*u2(jj)-u2p(jj)-4*y2p(jj)-y2(jj));       
    %Integro
